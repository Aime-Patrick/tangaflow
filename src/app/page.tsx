...
function AuthPage() {
  ...
  const navigate = (next: AuthView) => {
    const order: AuthView[] = ["login", "register", "forgot"];
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          {view === "register" && (
            <motion.div
              ...
              transition={{ duration: 0.3, ease: "easeInOut" }}
            >
              <RegisterForm onNavigate={navigate} />
            </motion.div>
          )}
          {view === "forgot" && (
            <motion.div
              ...
              transition={{ duration: 0.3, ease: "easeInOut" }}
            >
              <ForgotForm onNavigate={navigate} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
  ...
  return (
    <div className="rounded-none border border-border-subtle p-8">
      ...
      <p className="text-center text-sm text-text-muted mt-6">
        ...
        <button onClick={() => onNavigate("register")} className="text-text-primary hover:underline">
          Create one
        </button>
      </p>
    </div>
...
function RegisterForm({ onNavigate }: { onNavigate: (view: AuthView) => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    registerMutation.mutate({ email, password, name, orgName });
  };
  ...
  return (
    <div className="rounded-none border border-border-subtle p-8">
      ...
      <p className="text-center text-sm text-text-muted mt-6">
        ...
        <button onClick={() => onNavigate("login")} className="text-text-primary hover:underline">
          Sign in
        </button>
      </p>
    </div>
...
function ForgotForm({ onNavigate }: { onNavigate: (view: AuthView) => void }) {
  const [email, setEmail] = useState("");
